...
class PostProcessorFactory:
    ...
    def get_post_processor(job, postname):
        ...
        Path.Log.debug(f"Searching for post processor '{postname}' in paths: {paths}")
        ...
        for path in paths:
            ...
            if os.path.exists(module_path):
                 Path.Log.debug(f"Found candidate post script: {module_path}")
            ...
            if spec and spec.loader:
                ...
                try:
                    ...
                    Path.Log.error(f"Failed to load module {module_name}: {e}")
                    continue
