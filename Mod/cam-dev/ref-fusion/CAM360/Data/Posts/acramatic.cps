...
function getCommonCycle(x, y, z, r, w) {
  ...
  if ((currentSection.getPolarMode && currentSection.getPolarMode() != POLAR_MODE_OFF) && currentSection.isMultiAxis()) {
    ...
    return [xOutput.format(polarPosition.first.x), yOutput.format(polarPosition.first.y), zOutput.format(polarPosition.first.z),
      ...
      "R" + xyzFormat.format(r), "W" + xyzFormat.format(w)];
  } else {
    return [xOutput.format(x), yOutput.format(y), zOutput.format(z), "R" + xyzFormat.format(r), "W" + xyzFormat.format(w)];
  }
}
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(11)); // lock 4th axis
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(51)); // lock 5th axis
      }
    }
    return;
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(10)); // unlock 4th axis
      if (machineConfiguration.getNumberOfAxes() > 4) {
        writeBlock(fifthAxisClamp.format(50)); // unlock 5th axis
      }
    }
    return;
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      machineSimulation({
        ...
        coordinates: retract.retractAxes[2] ? MACHINE : WORK
      });
