...
var xyzFormat = createFormat({decimals:(unit == MM ? 3 : 4), type:FORMAT_REAL, minDigitsRight:(unit == MM ? 3 : 4)});
...
function onSection() {
  ...
  if (!state.retractedZ && xyzFormat.getResultingValue(getCurrentPosition().z) < xyzFormat.getResultingValue(initialPosition.z)) {
    ...
    machineSimulation({z:initialPosition.z});
  }
  ...
  if (state.retractedZ || isFirstSection() || true) { // we pull up Z above if we start lower
    ...
    machineSimulation({x:initialPosition.x, y:initialPosition.y, z:initialPosition.z});
  } else {
    writeBlock(
      gAbsIncModal.format(90),
      gMotionModal.format(0),
    ...
    machineSimulation({x:initialPosition.x, y:initialPosition.y});
  }
}
