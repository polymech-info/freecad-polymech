...
wcsDefinitions = {
  ...
  wcs          : [
    ...
    {name:"Extended", format:"G54 I", range:[0, 99]}
  ]
};
...
var ijkFormat = createFormat({decimals:6, type:FORMAT_REAL, scale:100}); // unitless
...
var toolVectorOutputI = createOutputVariable({prefix:"I1=", control:CONTROL_FORCE}, ijkFormat);
var toolVectorOutputJ = createOutputVariable({prefix:"J1=", control:CONTROL_FORCE}, ijkFormat);
var toolVectorOutputK = createOutputVariable({prefix:"K1=", control:CONTROL_FORCE}, ijkFormat);
...
var settings = {
  ...
  parametricFeeds: {
    ...
    feedAssignmentVariable: "E", // specifies the syntax to define a parameter
    feedOutputVariable    : "F=E" // specifies the syntax to output the feedrate as parameter
  },
...
function onOpen() {
  ...
  writeln("%PM" + getProgramNumber());
  writeln("N" + getProgramNumber() + " (" + getProgramNumber() + ")");
...
function onSection() {
  ...
  if (insertToolCall || !isFirstSection()) {
    ...
    wcsIsRequired = newWorkOffset || insertToolCall || isFirstSection();
  }
...
function cancelWorkPlane(force) {
  if (state.twpIsActive || force) {
    if (currentWorkPlaneXYZ.isNonZero()) {
      writeBlock(gFormat.format(93), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0), "Z" + xyzFormat.format(0));
      currentWorkPlaneXYZ = new Vector(0, 0, 0);
    }
    writeBlock(gFormat.format(7), "L1=0"); // cancel frame
...
function setWorkPlane(abc) {
  ...
  var workplaneIsRequired = (currentWorkPlaneABC == undefined) ||
    ...
    xyzFormat.areDifferent(currentSection.workOrigin.x, currentWorkPlaneXYZ.x) ||
    xyzFormat.areDifferent(currentSection.workOrigin.y, currentWorkPlaneXYZ.y) ||
    xyzFormat.areDifferent(currentSection.workOrigin.z, currentWorkPlaneXYZ.z);

  if (!workplaneIsRequired) {
    return; // Exit early if no change in workplane
  }
  writeStartBlocks(workplaneIsRequired, function () {
    ...
    if (currentSection.getId() > 0 && (isTCPSupportedByOperation(getSection(currentSection.getId() - 1) || tcp.isSupportedByOperation))) {
      setTCP(false); // cancel TCP
    }
    ...
    if (settings.workPlaneMethod.useTiltedWorkplane) {
      ...
      if (abc.isNonZero() || !machineConfiguration.isMultiAxisConfiguration()) {
        if (xyzFormat.areDifferent(currentSection.workOrigin.x, currentWorkPlaneXYZ.x) ||
            xyzFormat.areDifferent(currentSection.workOrigin.y, currentWorkPlaneXYZ.y) ||
            xyzFormat.areDifferent(currentSection.workOrigin.z, currentWorkPlaneXYZ.z)) {
          writeBlock(gFormat.format(93), "X" + xyzFormat.format(currentSection.workOrigin.x), "Y" + xyzFormat.format(currentSection.workOrigin.y), "Z" + xyzFormat.format(currentSection.workOrigin.z));
          currentWorkPlaneXYZ = currentSection.workOrigin;
        }
        writeBlock(
          gFormat.format(7), "A5=" + abcFormat.format(abc.x), "B5=" + abcFormat.format(abc.y), "C5=" + abcFormat.format(abc.z), "L1=1"
        ); // set frame and turn machine
...
function setTCP(_tcp) {
  ...
  writeBlock(_tcp ? gFormat.format(141) : gFormat.format(40));
