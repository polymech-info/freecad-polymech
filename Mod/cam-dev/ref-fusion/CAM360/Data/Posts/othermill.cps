...
function onOpen() {
  ...
  if (!getProperty("useToolCall")) {
    ...
    if (tools.getNumberOfTools() > 1) {
      error(localize("Only one tool allowed per program. Use property 'Use tool changer' if a tool changer is available."));
    }
  }
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    if (getProperty("useActiveSpindle")) {
      ...
      writeComment(formatWords(sOutput.format(spindleSpeed), mFormat.format(tool.clockwise ? 3 : 4)));
      onCommand(COMMAND_STOP);
    }
    return;
