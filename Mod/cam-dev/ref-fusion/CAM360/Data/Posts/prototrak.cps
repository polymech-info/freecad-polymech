...
circularOutputAccuracy = (unit == MM) ? 4 : 4;
circularInputTolerance = spatial(0.01, MM);
...
var xyzFormat = createFormat({decimals:(unit == MM ? 4 : 4), type:FORMAT_REAL});
...
var gCompModal = createOutputVariable({}, gFormat); // modal group 1 // G40-G42, ...
var gSmoothingModal = createOutputVariable({}, gFormat); // modal group // G61-G64
...
var settings = {
  ...
  maximumToolNumber           : 99, // the maximum tool number (Txxx), use 'undefined' for unlimited
...
function onSection() {
  ...
  if (hasParameter("operation:compensationType") && (getParameter("operation:compensationType") == "control") && (pendingRadiusCompensation != RADIUS_COMPENSATION_OFF)) {
    error(localize("Radius compensation 'In control' is not supported. Use 'Wear' instead."));
    return;
  }

  if (getProperty("useSmoothing")) {
    writeBlock(gSmoothingModal.format(61));
  } else {
    writeBlock(gSmoothingModal.format(64));
  }
  ...
  if (isFirstCyclePoint()) {
    ...
    case "counter-boring":
      writeBlock(
        ...
        getCommonCycle(x, y, z, cycle.retract), conditional(P > 0, "P" + milliFormat.format(P)),
        feedOutput.format(F)
      );
      break;
    case "chip-breaking":
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock(gMotionModal.format(1), gCompModal.format(41), x, y, z, getFeed(feed));
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock(gMotionModal.format(1), gCompModal.format(42), x, y, z, getFeed(feed));
        break;
      default:
        writeBlock(gMotionModal.format(1), gCompModal.format(40), x, y, z, getFeed(feed));
      }
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(circularOffset.x), jOutput.format(circularOffset.y), kOutput.format(0), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(circularOffset.x), jOutput.format(0), kOutput.format(circularOffset.z), getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(0), jOutput.format(circularOffset.y), kOutput.format(circularOffset.z), getFeed(feed));
      break;
    default:
      linearize(t);
    }
    ...
    case PLANE_XY:
      writeBlock(
        ...
        iOutput.format(circularOffset.x), jOutput.format(circularOffset.y), kOutput.format(0), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(
        gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3),
        xOutput.format(x), yOutput.format(y), zOutput.format(z),
        iOutput.format(circularOffset.x), jOutput.format(0), kOutput.format(circularOffset.z), getFeed(feed));
      break;
    case PLANE_YZ:
      writeBlock(
        gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3),
        xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(0),
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    machineSimulation({x:retract.positions.x, y:retract.positions.y, z:retract.positions.z, coordinates:MACHINE});
  }
}
...
function onClose() {
  ...
  if (getProperty("safePositionMethod") != "M22" && getProperty("useParkPosition")) {
    writeBlock(mFormat.format(22));
  }
