...
longDescription = "Generic post for Thermwood CNC routers.";
...
var xyzFormat = createFormat({decimals:(unit == MM ? 3 : 4), type:FORMAT_REAL}); // max 4
var abcFormat = createFormat({decimals:3, type:FORMAT_REAL, scale:DEG}); // max 4
var feedFormat = createFormat({decimals:1, type:FORMAT_REAL}); // only one decimal allowed
var toolFormat = createFormat({decimals:0});
var rpmFormat = createFormat({decimals:0});
var secFormat = createFormat({decimals:1, type:FORMAT_REAL}); // seconds - range 0.1-999.9
...
var settings = {
  ...
  maximumToolNumber             : 999, // specifies the maximum allowed tool number
  ...
  allowCancelTCPBeforeRetracting: true, // allows TCP/tool length compensation to be canceled prior retracting. Warning, ensure machine parameters are set to prevent axis motion when cancelling compensation.
...
function onOpen() {
  ...
  writeBlock(toolLengthCompOutput.format(46));
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || state.tcpIsActive || currentSection.isMultiAxis()) {
    ...
    if (insertToolCall || newWorkOffset || newWorkPlane) {
      insertToolCall = true; // force tool macro to make B axis origin point down
    }
  }
...
var toolLengthCompOutput = createOutputVariable({control : CONTROL_FORCE,
  onchange: function() {
    state.tcpIsActive = toolLengthCompOutput.getCurrent() == 48.1;
    state.lengthCompensationActive = (toolLengthCompOutput.getCurrent() == 47 || toolLengthCompOutput.getCurrent() == 48.1);
...
function getOffsetCode() {
  ...
  var offsetCode = tcp.isSupportedByOperation ? 48.1 : 47;
...
function disableLengthCompensation(force) {
  if (state.lengthCompensationActive || force) {
    ...
    if (state.tcpIsActive) {
      var saveMinDigitsRight = toolLengthCompOutput.getMinDigitsRight();
      toolLengthCompOutput.setMinDigitsRight(1); // required to output G48.0
      writeBlock(toolLengthCompOutput.format(48.0));
      toolLengthCompOutput.setMinDigitsRight(saveMinDigitsRight);
    } else {
      writeBlock(toolLengthCompOutput.format(46));
    }
  }
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  writeStartBlocks(isRequired, function() {
    ...
    writeBlock(gMotionModal.format(motionCode.single), zOutput.format(position.z), hOffset, additionalCodes[1]);
...
function onDwell(seconds) {
  ...
  seconds = clamp(0.1, seconds, maxValue);
...
function onCyclePoint(x, y, z) {
  if (!getProperty("useCycles") || (!isSameDirection(machineConfiguration.getSpindleAxis(), getForwardDirection(currentSection)))) {
    if (isTappingCycle()) {
  ...
  if (isFirstCyclePoint() || cycleRequiresRetract) {
    ...
    case "counter-boring":
      writeBlock(
        gAbsIncModal.format(90), gCycleModal.format(P > 0 ? 82 : 81),
        getCommonCycle(x, y, z, cycle.retract),
        P > 0 ? "P" + secFormat.format(P) : "", // not optional
        feedOutput.format(F)
      );
      break;
    case "deep-drilling":
    ...
    case "right-tapping":
      ...
      var tappingCode = (tool.type == TOOL_TAP_LEFT_HAND || cycleType == "left-tapping") ? 74 : 84;
      writeBlock(
        gAbsIncModal.format(90), gCycleModal.format(tappingCode),
        getCommonCycle(x, y, z, cycle.retract),
        feedOutput.format(F)
      );
      break;
    ...
    case "boring":
      ...
      writeBlock(
        gAbsIncModal.format(90), gCycleModal.format(P > 0 ? 89 : 85),
        getCommonCycle(x, y, z, cycle.retract),
        P > 0 ? "P" + secFormat.format(P) : "", // not optional
        feedOutput.format(F));
      break;
...
function onCommand(command) {
  ...
  case COMMAND_START_SPINDLE:
    ...
    writeBlock(sOutput.format(spindleSpeed), conditional(gotProgrammableSpindle, mFormat.format(tool.clockwise ? 3 : 4)));
    return;
...
function onSectionEnd() {
  writeBlock(gPlaneModal.format(17));
  if (machineConfiguration.isMultiAxisConfiguration() && (machineConfiguration.getAxisV().getCoordinate() == 2) && machineConfiguration.getAxisV().isCyclic()) {
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      case "G990":
        forceModals(gMotionModal);
        writeBlock(gFormat.format(990)); // reset / use machine coordinates
        currentWorkOffset = undefined; // need to force the wcs after setting G990
