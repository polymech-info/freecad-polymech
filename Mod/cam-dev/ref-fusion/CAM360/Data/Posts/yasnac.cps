...
var toolFormat = createFormat({minDigitsLeft:4, decimals:0}); // change to 2 for old controls
var rpmFormat = createFormat({decimals:0});
var pitchFormat = createFormat({decimals:(unit == MM ? 3 : 4)});
var secFormat = createFormat({decimals:3, type:FORMAT_REAL}); // seconds - range 0.001-999999.999
...
function onOpen() {
  ...
  writeln("%");
  if (typeof oFormat != "undefined" && getProperty("o5")) {
    oFormat.setMinDigitsLeft(5);
    settings.programNumber = {max:99999};
  }
...
function onDwell(seconds) {
  ...
  writeBlock(gFormat.format(4), "P" + (seconds >= 1 ? secFormat.format(seconds) : milliFormat.format(seconds * 1000)));
}
